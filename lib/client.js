...
class Client {
  ...
  restList(projectName, resName, filter = {}) {
    return this._request('GET', projectName, '/' + resName, filter)
  }

  restGet(projectName, resName, id) {
    return this._request('GET', projectName, `/${resName}/${id}`, {})
  }

  restCreate(projectName, resName, data) {
    const body = Buffer.from(JSON.stringify(data));
    const headers = {
      'x-log-bodyrawsize': body.byteLength
    };
    return this._request('POST', projectName, '/' + resName, {}, body, headers);
  }

  applyConfigToMachineGroup(projectName, groupName, configName) {
    return this._request('PUT', projectName, `/machinegroups/${groupName}/configs/${configName}`, {}, Buffer.from('{}'))
  }
}
